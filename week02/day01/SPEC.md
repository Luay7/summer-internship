...
* **URL Input:** The tool accepts a single target web page URL via the command-line interface (CLI) to initiate the scan.
...
* **Structured Log Reporting:** The tool outputs a clean, unified log-style report in the terminal. Each entry includes the exact timestamp, the final classification of the link, the URL itself, and the sequence of requests made (e.g., `[YYYY-MM-DD HH:MM:SS] [Protected] https://example.com | HEAD: 403 -> GET: 200`).
